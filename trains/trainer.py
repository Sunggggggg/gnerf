...
class Trainer(object):
    ...
    def make_video(self, nframes=40):
        ...
        imageio.mimwrite(os.path.join(self.video_dir, f'rgb_{self.it:04}.gif'), rgbs, duration=10)

        depths = (depths.cpu().permute(0, 2, 3, 1).numpy().clip(0, 1) * 255).astype(np.uint8)
        imageio.mimwrite(os.path.join(self.video_dir, f'depth_{self.it:04}.gif'), depths, duration=10)
